...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        if len_result != 1:
            result[0].append(
                [
                    InlineKeyboardButton("ඊලඟ ⏩", callback_data=f"navigate(0|next|{query})")
                ]
            )
        
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"📃 පිටුව 1/{len_result if len_result < max_pages else max_pages} 📃", callback_data="ignore")
        ])
        ...
        try:
            await bot.send_message(
                ...
                text=f"ඔයා හොයපු එකෙන් Results {(len_results)} ක් හමුවුනා 👇: <code>{query}</code>",
